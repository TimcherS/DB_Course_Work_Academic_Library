...
async def get_book(book_id: int, db: Session = Depends(get_db)):
    try:
        ...
        return {
            ...
            "isbn": book.isbn or "",
            "release_date": book.release_date.year if book.release_date else "",
...
async def add_book(book_data: dict, db: Session = Depends(get_db)):
    try:
        ...
        if not shelf_location:
            raise HTTPException(status_code=400, detail="Местоположение обязательно")

        # Преобразуем типы данных
        if release_date:
            try:
                year_val = int(release_date)
                release_date = datetime.datetime(year_val, 1, 1)
            except ValueError:
                release_date = None
        ...
        for _ in range(number_of_books):
            book_item = BookItem(
                ...
                write_off_reasons=None
            )
            book_item.shelf_location = shelf_location
            book_item.book_price = int(price) if price else None
            db.add(book_item)
...
async def loan_book_to_reader(book_id: int, loan_data: dict, db: Session = Depends(get_db)):
    ...
    # Ловим ошибку от триггера (RAISE EXCEPTION в Postgres)
