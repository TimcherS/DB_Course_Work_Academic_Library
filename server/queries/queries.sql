...
-- Применение реляционных и булевых операторов в предикате
SELECT * FROM books
JOIN themes ON books.theme_id = themes.theme_id
WHERE theme_name = 'Математика' OR theme_name = 'Химия';

-- Выборка книг по теме Экономики/Социологии/Инязу либо выборка книг выпущенных в 2000 году
-- Применение Union
...
-- Выборка книг по теме Математике авторства 'Джерард В.' но только те что вышли в 1700 году
-- Применение EXCEPT и INTERSECT
SELECT books.* FROM books
JOIN themes ON books.theme_id = themes.theme_id
...
-- Применение GROUP BY
-- Использование групповых (аггрегирующих) функций
...
-- Регистрация книг (ОК)
-- Использование в одном запросе нескольких таблиц
-- 
BEGIN;
do $$
begin 
...
-- Применение CASE для построения вычислимого поля с форматированием результата
