    ...
    try:
        ...
        for loan in loans:
            ...
            if book_item:
                ...
                if book:
                    # Приводим даты к date для сравнения
                    loan_due_date = loan.loan_due_date.date() if isinstance(loan.loan_due_date, datetime) else loan.loan_due_date
                    loan_return_date = loan.loan_return_date.date() if isinstance(loan.loan_return_date, datetime) else loan.loan_return_date
                    
                    # Определяем статус займа
                    status = "Возвращена" if loan_return_date else "На руках"
                    if loan_return_date and loan_return_date > loan_due_date:
                        status = "Возвращена с опозданием"
                    elif not loan_return_date and loan_due_date < datetime.now().date():
                        status = "Просрочена"
